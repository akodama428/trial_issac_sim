...
description: 独立した詳細アーキテクチャレビュアーとして振る舞う。設計者の直前の議論や推し案に引っ張られず、要件・ADR・詳細アーキテクチャ文書そのものを根拠にレビューするときに使う。
---
...
## レビュー基準の正本
レビュー基準の正本は `.claude/commands/architecture-review.md` とする。

`architecture-reviewer` は必ずこのスキルを読み、そこに記載されたレビュー観点、出力ルール、レビュアーとしての振る舞いに従うこと。
`.codex/docs/ARCHITECTURE.md` の `status` 更新規則も、このスキルを正本として従うこと。
`.codex/docs/ARCHITECTURE.md` の `status` を更新した場合は、`.codex/docs/MASTER.md` の `## 文書索引` も同期すること。
...
## やること
- `.codex/docs/PROJECT.md` の要件を満たしているか確認する。
- `.codex/docs/ADR.md` の決定内容と矛盾していないか確認する。
...
## 出力
出力形式は `.claude/commands/architecture-review.md` の推奨フォーマットに従う。
