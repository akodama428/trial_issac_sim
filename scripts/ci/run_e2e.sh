...
ISAAC_SIM_GID="$(
  docker run --rm "${IMAGE_NAME}" bash -c 'id -g isaac-sim' 2>/dev/null \
    | tr -d '[:space:]' \
    || true
)"
...
docker run --rm \
  ...
  -e CI_RECORD_HOME_DIVERGENCE_BAG="${CI_RECORD_HOME_DIVERGENCE_BAG:-}" \
  ...
  -e TOMATO_HARVEST_GRASP_DIRECT_JTC="${TOMATO_HARVEST_GRASP_DIRECT_JTC:-}" \
  -e TOMATO_HARVEST_FRICTION_HOLD_EVAL_STEPS="${TOMATO_HARVEST_FRICTION_HOLD_EVAL_STEPS:-}" \
  -e TOMATO_HARVEST_FRICTION_HOLD_EVAL_MIN_LIFT_M="${TOMATO_HARVEST_FRICTION_HOLD_EVAL_MIN_LIFT_M:-}" \
  -e TOMATO_HARVEST_STEM_BREAK_NON_PULL_STEPS="${TOMATO_HARVEST_STEM_BREAK_NON_PULL_STEPS:-}" \
  -e TOMATO_HARVEST_PLACEMENT_TEST_OFFSET_X_M="${TOMATO_HARVEST_PLACEMENT_TEST_OFFSET_X_M:-}" \
  -e CI_EXPECT_PLACEMENT="${CI_EXPECT_PLACEMENT:-placed}" \
