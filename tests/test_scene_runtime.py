...
class SceneRuntimeLayoutTest(unittest.TestCase):
    ...
    def test_close_gripper_clears_active_motion_target_for_gripper_phase(self) -> None:
    ...
    def test_open_gripper_clears_active_motion_target_for_gripper_phase(self) -> None:
        runtime = IsaacSceneRuntime(physics_grasp_enabled=True)
        runtime.boot()
        target_pose = Pose3D(0.55, 0.15, 0.45, 180.0, 0.0, 0.0)

        runtime.set_place_pose(target_pose, waypoint_poses=(target_pose,))
        snapshot = runtime.open_gripper()
