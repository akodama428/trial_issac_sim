...
- CI E2E runnerでは`TOMATO_HARVEST_SERVO_MODE`をlaunch引数へ透過する。既定値は`off`である。
...
launch interface確認では`servo_mode`の既定値が`off`、有効値が`off`、`jtc`として公開されることをROS 2 Jazzy環境で確認した。
...
現段階では既存実装を削除しない。`ServoExecutionAdapter`を実装して両経路ともE2E完了したが、Servo経路はtarget_foundからcompleteまで79.6%遅く、非劣化Gateを満たさなかった。tracking error peakは改善したため制御の滑らかさには利点があるが、速度劣化を許容して即時置換する根拠にはしない。`safe_online_solver.py`、`local_planner_node`、`motion_command_executor_node`は維持し、既定値も`off`のままとする。
