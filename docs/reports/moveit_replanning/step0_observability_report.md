...
Step 0 で特定した原因に対し、stop trajectory と home trajectory の開始点を arm JTC が
管理する7関節へ名前ベースで射影した。修正後に同じ900-step E2Eを再実行した結果は次の
とおりだった。
...
1. arm JTC へ渡す stop/home trajectory から finger joints を分離した。後続ステップでも
   arm trajectory と gripper command の契約を混在させない。
