...
flowchart TB
  subgraph EXEC_NODE["ROS node: motion_command_executor_node"]
    ...
    GEN["goal generation<br/>旧callbackを無視"]:::changed
...
### 原因2: executorの旧goal callbackが新goal状態を破壊する

executorはgoal置換時に旧goalを非同期cancelした直後、新goalを送る。旧goalのresult callbackが遅れて到着すると、従来は共有`goal_handle_`を無条件にresetしたため、新goal handleまで失った。JTC自体は新goalを実行していても、次の置換でexecutorがactive goalをcancelできず、観測状態と実行状態が不一致になる競合だった。
...
|---|---:|---:|---|
| `open_loop_control` | true | false | deprecated設定を外し、実測stateを利用 |
...
変更後matrixでは101 goalを送信し、61回の意図的なtrajectory replacementを含んだ。action feedbackまで実行対象として残った61 goalで`trajectory_command_reflected`を61件記録し、旧callbackによるactive handle喪失、JTC abort、指令凍結は観測されなかった。local補正回数とlive sample数の差は初期姿勢ごとの実軌道・追従誤差によるrun間変動で、publish/adopt不一致や成功率低下はない。
...
指令凍結は「JTCが新goalを拒否する」単独障害ではなく、関節限界近傍でdesired/actualが乖離した後に古いcommandを補間seedとして再利用する設定と、executorの非同期callback世代競合が回復を不安定にする複合条件だった。新goalを実測stateから開始し、旧callbackを隔離し、controller stateで診断を補完することで、制御責務を保ったまま新goal反映を観測可能にした。
