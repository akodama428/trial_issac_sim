...
| 実行日 | 2026-07-11 |
| unit test | Python 125件 (新規16件を含む)、全件成功 |
...
未知の `producer_kind` / `planned_from_phase` 値をエラーにせず安全側 (`UNKNOWN` / `None`) へ
落とす。これにより新旧ノード混在時もデシリアライズは失敗しない。
...
1. **producer 規則**: `producer_kind` が既知 (`global_planner` / `local_planner`) でない plan は
   採用しない → `rejected_unknown_producer`
2. **旧契約互換**: `plan_revision == 0` (未版数) は従来挙動どおり常に採用 → `adopted_legacy_contract`
3. **revision 規則**: 採用済み revision 以下は stale として棄却 → `rejected_stale_revision`
4. **phase 整合規則**: 実行 phase (`moving_to_*` / `detaching` / `returning_home`) 起点の replan は
   ...
   full-chain plan は phase-bound にしない
...
| revision / phase / legacy / producer の採用規則 | `robot/execute_manager/tests/test_plan_adoption.py` (10件) |
| 再現シナリオの期待一致と抑止数 | `tests/test_plan_adoption_stale_demo.py` (2件) |

Python unit test は合計 125 件すべて成功した。

## 7. 次ステップへ渡す判断材料

1. 採用規則は planner 実装から独立した pure function であり、Step 2 の trigger policy /
   state aggregation の切り出し先からそのまま再利用できる。
2. `producer_kind` は判定へ組み込み済みのため、Step 5 で local planner を追加しても
   consumer 側の契約変更は不要である。
...
- 旧契約互換 (`plan_revision == 0` を常に採用) は producer 更新完了後に削除できる。
