...
## 追加検証: E2E再ビルド条件の統一（2026-07-17）

### 背景

GUI手動実行の`./scripts/run_ros2_components.sh --isaac --moveit --rebuild --grasp-mode physics`では、トマトをトレーまで保持して運べることが確認された。一方、従来E2Eはphysicsモードではあったが`--rebuild`を指定していなかった。この差を除くため、`scripts/ci/in_container_e2e.sh`からも常に`--rebuild`を渡し、中央把持48 mm・physics・headless 3,600 stepを再実行した。

ログ上で以下を確認した。

- C++ package `franka_ros2_control`を実際に再ビルド（1 package finished）
- Isaac起動引数は`--headless --headless-steps 3600 --grasp-mode physics`
- `grasp_mode=physics`
- physics tuningは120 Hz、TGS、solver 32/4、finger drive 15 N

### rebuildなし／ありの比較

| 指標 | 中央把持・rebuildなし | 中央把持・rebuildあり |
|---|---:|---:|
| HELD継続 | 50 sample / 0.99 s | 47 sample / 0.91 s |
| HELD中最大速度 | 0.173 m/s | **0.167 m/s** |
| DETACHED継続 | 13 sample / 0.24 s | **33 sample / 1.05 s** |
| DETACHED中最大速度 | 0.621 m/s | **0.450 m/s** |
| 状態列 | `ATTACHED → HELD → DETACHED → FALLEN` | 同左 |
| phase終端 | `moving_to_place → failed` | 同左 |

再ビルドによりDETACHED観測時間は約4.4倍へ延び、速度ピークも低下したが、E2Eのcycle completionは得られなかった。したがって、古いC++ buildだけが従来失敗の原因ではない。

### FALLEN判定と実物理観測の不整合

rebuildありrunの`FALLEN`直前・直後には、以下が同時に成立していた。

- 左右finger contactが継続
- 左右接触力が約16 Nで継続
- finger gapが0.0200 mで継続
- finger–tomato上下差が+0.4〜+0.5 mmで継続
- hand–tomato距離が0.1036 mで一定

つまり、状態機械は`FALLEN`を報告したが、トマトは両指間の中央位置と接触を維持していた。これはGUI手動実行でトレーまで運べた観察とも整合する。

現在の`FrictionGraspStrategy`は、把持成立時の`tomato_position - hand_position`を**world座標ベクトル**として保存し、その後のworld座標ベクトルとの差が5 mmを超えると`LOST`にする。handとtomatoが一体で移動して距離を維持していても、手首姿勢変更で相対ベクトルのworld方向が回転すれば差分が発生する。このため、今回の`FALLEN`は実滑落ではなく、回転非不変な滑り判定による偽陽性である可能性が高い（上記観測からの推定）。

### 再評価の結論

- E2Eは従来から正しくphysicsモードだった。
- `--rebuild`統一は再現条件として必要であり、E2E既定へ追加した。
- rebuildにより保持観測は改善したが、現在の`FALLEN`判定は物理接触ログと矛盾する。
- 次の最優先修正はpull速度変更ではなく、滑り量をhand local frameまたはfinger接触点に対する相対位置で評価し、手首回転と実滑りを分離することである。

再現ログ: `.artifacts/step3-7/vertical-centered-rebuild/e2e/`
